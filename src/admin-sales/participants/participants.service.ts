...
import { ParticipantResponse } from './interfaces/participants-actives-response.interface';
import { formatParticipantResponse } from './helpers/format-participants-actives-response.helper';
...
export class ParticipantsService {
  ...
  async create(createParticipantDto: CreateParticipantDto): Promise<ParticipantResponse> {
    const existingParticipant = await this.participantRepository.findOne({
      where: { document: createParticipantDto.document }
    });
    if (existingParticipant)
      throw new BadRequestException(`Ya existe un participante con el documento ${createParticipantDto.document}`);
  ...
  async update(id: string, updateParticipantDto: UpdateParticipantDto): Promise<ParticipantResponse> {
    ...
    if (updateParticipantDto.document && updateParticipantDto.document !== existingParticipant.document) {
      const duplicateDocument = await this.participantRepository.findOne({
        where: { document: updateParticipantDto.document }
      });
      if (duplicateDocument)
        throw new BadRequestException(`Ya existe un participante con el documento ${updateParticipantDto.document}`);
    }
  ...
  async validateParticipantByType(participantId: string, expectedType: string): Promise<void> {
    const participant = await this.participantRepository.findOne({
      where: { id: participantId }
    });
    if (!participant) {
      throw new NotFoundException(`Participante con ID ${participantId} no encontrado`);
    }
  ...
  const whereCondition = type ? { participantType: type } : {};
