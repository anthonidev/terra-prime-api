...
export class ChatbotService {
  ...
  private async generateOptimizedResponse(
    ...
    try {
      ...
      // 🎯 Construir prompt ultra optimizado CON CONTEXTO ESPECÍFICO
      ...
      return this.cleanAndValidateResponse(response, user.firstName);
    } catch (error) {
      this.logger.error(
  ...
  /**
   * 🧹 Limpia y valida la respuesta de Claude
   */
  private cleanAndValidateResponse(
    response: string,
    firstName: string,
  ): string {
    // Limpiar respuesta
    ...
    // Validar que tenga emojis (característica clave de SmartBot)
    const hasEmojis =
    ...
    if (!hasEmojis) {
      // Agregar emoji al inicio si no tiene
      cleanResponse = `🤖 ${cleanResponse}`;
    }

    // Validar longitud mínima
    if (cleanResponse.length < 20) {
      return `🤖 ${firstName}, necesito más información para ayudarte mejor. ¿Podrías ser más específico? 😊`;
    }
