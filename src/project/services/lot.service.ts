...
export class LotService {
  ...
  async findLotsByProjectId(
    ...
    if (stageId) queryBuilder.andWhere('stage.id = :stageId', { stageId });

    if (blockId) queryBuilder.andWhere('block.id = :blockId', { blockId });
  ...
  async findAllByBlockId(blockId: string): Promise<LotResponse[]> {
    ...
    if (!lots || lots.length === 0)
      throw new NotFoundException(
        `No se encontraron lotes activos para esta manzana`,
      );
  ...
  async isLotValidForSale(lotId: string): Promise<Lot> {
    ...
    if (!lot) throw new NotFoundException(`El lote no se encuentra registrado`);
    if (lot.status === LotStatus.INACTIVE)
      throw new BadRequestException(
        `El lote ${lot.name} no se encuentra activado`,
      );
  ...
  async isLotValidForSaleReservation(lotId: string): Promise<Lot> {
    ...
    if (!lot)
      throw new NotFoundException(
        `El lote ${lot.name} no se encuentra separado`,
      );
    if (lot.status === LotStatus.INACTIVE)
      throw new BadRequestException(
        `El lote ${lot.name} no se encuentra activado`,
  ...
  async updateStatus(id: string, status: LotStatus, queryRunner?: QueryRunner) {
  ...
  async createPinBySalesManager(userId: string): Promise<{ pin: string }> {
